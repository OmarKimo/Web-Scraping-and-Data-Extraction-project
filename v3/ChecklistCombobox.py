...
class ChecklistCombobox(ttk.Combobox):
    ...
    def __init__(self,master=None,**kw):
        ...
        if not isinstance(self.values,(list,tuple)): self.values = list(self.values)
    ...
    def on_checkbutton_click_release(self,event):
        ...
        if variable.get() == 0:
            if not idx:
                for but in reversed(self.checkbuttons):
                    but.select()
                    self.last_clicked_button = but
    ...
    def configure_popdown(self,event=None):
        ...
        self.canvas_frame.config(height=visible_height+2*sum(borders[1:3]))
